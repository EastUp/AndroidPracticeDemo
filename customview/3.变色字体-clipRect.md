...
**canvas.save()//先保存画布的状态,防止从左向右裁剪后,再从右向左裁剪没效果**

**canvas.clipRect(float left, float top, float right, float bottom) //裁剪画布**

**canvas.restore()//因为实在clipRect之前save了的,所以restore后画布还是完整的**
