package com.east.androidplugindevelop.activity.hook.dex;
 ...
 * |---------------------------------------------------------------------------------------------------------------|
 */
public class LoadPluginDexManager {

    private Context mApplicationContext;
    private File mOwnDexDir; // 应 用可以访问的dex目录


    public LoadPluginDexManager(Context context) {
     ...
     * @param pluginDexPath
     */
    public void loadPlugin(String pluginDexPath) throws Exception{
        // 1. 将下载好的补丁文件拷贝到系统可以访问的dex目录下

        File srcDexFile = new File(pluginDexPath);
        if(!srcDexFile.exists()){
            throw new RuntimeException("patch [" + pluginDexPath + "] has be loaded.");
        }
        File destDexFile = new File(mOwnDexDir,srcDexFile.getName());

        if (destDexFile.exists()) {
     ...
     * 修复加载 所有的补丁包
     */
    private void loginPluginFiles(List<File> pluginDexFiles) throws Exception{
        ...
        for (File fixDexFile : pluginDexFiles) {
