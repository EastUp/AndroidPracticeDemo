...
class StubContentProvider : ContentProvider() {
    ...
    override fun onCreate(): Boolean {
        uriMatcher?.addURI("com.east.stubprovider", "plugin1", 0)
        uriMatcher?.addURI("com.east.stubproviderr", "plugin2", 0)
        return true
    }
    ...
    private fun loadPluginProvider() {
        if (pluginProvider == null) {
            pluginProvider = PluginUtils.classLoader?.loadClass("com.east.plugin.PluginContentProvider")?.newInstance() as ContentProvider?
        }
    }
