...
public class Hermes {
    ...
    //执行单例对象的方法
    public static final int TYPE_INVOKE_METHOD = 0;
    //得到单例
    ...
    public <T> T getInstance(Class<T> tClass, Object... parameters) {
        Responce responce = sendGetObjRequest(HermesService.class, tClass, null, parameters);
    ...
    private <T> Responce sendGetObjRequest(Class<HermesService> hermesServiceClass,
    ...
    public <T> Responce sendInvokeMethodRequest(Class hermeService, Class<T> aClass,
        ...
        Request request = new Request(gson.toJson(requestBean), TYPE_INVOKE_METHOD);
