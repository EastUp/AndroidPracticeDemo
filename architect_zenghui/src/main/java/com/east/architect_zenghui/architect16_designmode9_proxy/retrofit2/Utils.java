...
final class Utils {
	...
	static Type resolve(Type context, Class<?> contextRawType, Type toResolve) {
		...
		while (true) {
			if (toResolve instanceof TypeVariable) {
				...
				return changed ? new ParameterizedTypeImpl(newOwnerType,
				...
				if (originalLowerBound.length == 1) {
					...
					if (upperBound != originalUpperBound[0]) {
						return new WildcardTypeImpl(new Type[] { upperBound },
								EMPTY_TYPE_ARRAY);
					}
				}
				return original;
	...
	private static Type resolveTypeVariable(Type context,
			Class<?> contextRawType, TypeVariable<?> unknown) {
	...
	static ResponseBody buffer(final ResponseBody body) throws IOException {
		...
		return ResponseBody.create(body.contentType(), body.contentLength(),
				buffer);
	}
	...
	static boolean hasUnresolvableType(Type type) {
		...
		if (type instanceof GenericArrayType) {
			return hasUnresolvableType(((GenericArrayType) type)
					.getGenericComponentType());
		}
		if (type instanceof TypeVariable) {
			return true;
	...
	private static final class WildcardTypeImpl implements WildcardType {
		...
		public boolean equals(Object other) {
			return other instanceof WildcardType
					&& Utils.equals(this, (WildcardType) other);
		}
